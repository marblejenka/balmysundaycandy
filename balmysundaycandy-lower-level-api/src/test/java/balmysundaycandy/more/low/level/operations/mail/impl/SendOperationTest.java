...
public class SendOperationTest extends MailTestCase {
	@Test
	public void testCallMailMessage() {
		MailMessage request = new MailMessage();
		{
			...
			request.addTo(ApiProxy.getCurrentEnvironment().getEmail());

			request.setSubject("this is unit test from appengine");
			request.setHtmlBody("body");
		}

		VoidProto response = MailOperations.SEND.call(request);
	...
	public void testCallAsyncMailMessageApiConfig() throws InterruptedException, ExecutionException {
		...
		Future<VoidProto> response = MailOperations.SEND.callAsync(request, new ApiConfig());

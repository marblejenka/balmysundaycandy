 ...
 * @author marblejenka
 */
public final class DeleteIndexOperation extends DatastoreOperation<CompositeIndex, VoidProto> {

	@Override
	public VoidProto call(CompositeIndex request) {
		VoidProto voidProto = new VoidProto();
		voidProto.mergeFrom(ApiProxy.makeSyncCall(datastore_v3.packageName, datastore_v3.methodName.DeleteIndex, request.toByteArray()));
		return voidProto;
	}
	...
	public Future<VoidProto> callAsync(CompositeIndex request, ApiConfig apiConfig) {
		...
		return new ProtocolMessageFuture<VoidProto>(ApiProxy.makeAsyncCall(datastore_v3.packageName, datastore_v3.methodName.DeleteIndex, request.toByteArray(), apiConfig),
				response);
	}
}
...
public final class SendOperation extends MailOperation<MailMessage, VoidProto> {

	@Override
	public VoidProto call(MailMessage request) {
		VoidProto response = new VoidProto();
		response.mergeFrom(ApiProxy.makeSyncCall(mail.packageName, mail.methodName.Send, request.toByteArray()));
		return response;
	}
